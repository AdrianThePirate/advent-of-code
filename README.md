...
All solutions can be run from the root of the repo:

```
go run . <year> <day> [variant]
```

The optional `variant` selects which input file to use. Omitting it uses the real puzzle input.

```
go run . 2024 1            # real input  (day1.txt)
go run . 2024 1 sample     # sample input (day1_sample.txt)
go run . 2024 16 sample1   # named sample (day16_sample1.txt)
go run . 2024 16 sample2   # named sample (day16_sample2.txt)
```

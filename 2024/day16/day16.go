...
func Run(variant string) {
	path := cmd.InputPath("2024/day16/day16", variant)
	...
	for scanner.Scan() {
		for x, r := range scanner.Text() {
			switch r {
			case '#':
				walls[vector.Vec2[int]{X: x, Y: y}] = true
			case 'S':
				rain = position{pos: vector.Vec2[int]{X: x, Y: y}, deg: 1}
			case 'E':
				goal = vector.Vec2[int]{X: x, Y: y}
			}
		}
		y++
	}
...
func aStarAlg(rain position, goal vector.Vec2[int], walls map[vector.Vec2[int]]bool) (int, int) {
	...
	for len(openSet) > 0 {
		...
		case 0:
			options = append(options, position{pos: current.pos.pos.Up(), deg: current.pos.deg})
		case 1:
			options = append(options, position{pos: current.pos.pos.Right(), deg: current.pos.deg})
		case 2:
			options = append(options, position{pos: current.pos.pos.Down(), deg: current.pos.deg})
		case 3:
			options = append(options, position{pos: current.pos.pos.Left(), deg: current.pos.deg})
		}
		options = append(options, position{pos: current.pos.pos, deg: (current.pos.deg - 1 + 4) % 4})
		options = append(options, position{pos: current.pos.pos, deg: (current.pos.deg + 1 + 4) % 4})

		for _, neighPos := range options {
			if closedSet[neighPos] {
				continue
			}

			if walls[neighPos.pos] {
				continue
			}

			tentG := current.g + 1
			if neighPos.deg != current.pos.deg {
				tentG = current.g + 1000
			}
			neigh := node{
				pos:    neighPos,
				g:      tentG,

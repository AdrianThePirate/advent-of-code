...
func Run(variant string) {
	path := cmd.InputPath("2024/days/day12/day12", variant)
	farm, err := input.FileToArray2D[rune](path)
	if err != nil {
		fmt.Println(err)
		return
	}
...
func fencePrice(farm array.Array2D[rune]) (int, int) {
	...
	for y, l := range farm {
		for x, r := range l {
			...
			if r != '.' {
				if area, border, err := sizeRegion(&farm, pos, &v); err == nil {
					price += area * border
					priceBulk += area * findEdges(v)
				}
			}
		}
	}
...
func sizeRegion(farm *array.Array2D[rune], pos vector.Vec2[int], visited *[]vector.Vec2[int]) (int, int, error) {
	...
	*visited = append(*visited, pos)
	...
	area += 1
	if val, err := farm.GetPos(pos.Up()); err == nil && val == t {
		if a1, b1, err := sizeRegion(farm, pos.Up(), visited); err == nil {
			area += a1
			border += b1
		}
	} else if !slices.Contains(*visited, pos.Up()) {
		border += 1
	}
	if val, err := farm.GetPos(pos.Down()); err == nil && val == t {
		if a1, b1, err := sizeRegion(farm, pos.Down(), visited); err == nil {
			area += a1
			border += b1
		}
	} else if !slices.Contains(*visited, pos.Down()) {
		border += 1
	}
	if val, err := farm.GetPos(pos.Left()); err == nil && val == t {
		if a1, b1, err := sizeRegion(farm, pos.Left(), visited); err == nil {
			area += a1
			border += b1
		}
	} else if !slices.Contains(*visited, pos.Left()) {
		border += 1
	}
	if val, err := farm.GetPos(pos.Right()); err == nil && val == t {
		if a1, b1, err := sizeRegion(farm, pos.Right(), visited); err == nil {
			area += a1
			border += b1
		}
	} else if !slices.Contains(*visited, pos.Right()) {
		border += 1
	}

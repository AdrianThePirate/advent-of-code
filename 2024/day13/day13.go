...
func Run(variant string) {
	path := cmd.InputPath("2024/day13/day13", variant)
	...
	var arr array.Array[struct {
		vec1 vector.Vec2[int64]
		vec2 vector.Vec2[int64]
		pos  vector.Vec2[int64]
	}]
	var arr2 array.Array[struct {
		vec1 vector.Vec2[int64]
		vec2 vector.Vec2[int64]
		pos  vector.Vec2[int64]
	}]
	for scanner.Scan() {
		...
		arr = append(arr, struct {
			...
			vec2 vector.Vec2[int64]
			pos  vector.Vec2[int64]
		}{vec1: vec1, vec2: vec2, pos: pos})
		arr2 = append(arr2, struct {
			vec1 vector.Vec2[int64]
			vec2 vector.Vec2[int64]
			pos  vector.Vec2[int64]
		}{vec1: vec1, vec2: vec2, pos: pos2})
	}
...
func findTokenPrice(arr array.Array[struct {
	vec1 vector.Vec2[int64]
	vec2 vector.Vec2[int64]
	pos  vector.Vec2[int64]
}]) (int64, error) {
